...
class C_kegiatan_jabatan extends CI_Controller {
    ...
    public function detail2() {
		...
		$get=$this->db->where('kodejab',$jabatan)->get('tbljabatan')->row();
		if($get->jenis=='JST'){
			$data['arrJabatan']=array();
		}else{
        $data['arrJabatan']=$this->db->query("select * from tbljabatan where jenis='JFT' AND kelas=(
		  select b.kelas from dd_user a
            inner join tbljabatan b on a.jabatan=b.kodejab
...
//output to json format
    ...
    public function ajax_list_detail2() {
        ...
        $kodejab = $this->input->post('kodejab');
        $list = $this->kegiatan_jabatan->get_datatables($kodejab,$nama);
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $dt) {
            $no++;
...
<i class="fa fa-copy text-success"/></a>';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->kegiatan_jabatan->count_all($kodejab,$nama),
            "recordsFiltered" => $this->kegiatan_jabatan->count_filtered($kodejab,$nama),
            "data" => $data,
        );
//output to json format
        echo json_encode($output);
