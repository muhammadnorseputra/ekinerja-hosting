...
class C_api extends CI_Controller {
    ...
    function get_skp_tahunan() {
        ...
        if ($nilai) {
            ...
            $posts[] = array(
                    ...
                    "tahun"               =>  "",
                    "bulan"               =>  "",
                    "nilai_skp"           =>  "Data Tidak Ditemukan"
            );
            header('Content-Type: application/json');
            echo json_encode($posts,TRUE);
        }        
    ...
    // untuk webservice ke ANDROID, hasilnya json dengan ARRAY 1 LAPIS
    ...
    function get_skp_periodenip_silka() {
        //http://localhost/expneo-baru/index.php/c_api/get_skp_periodenip_silka?thn='.$thn.'&nip='.$nip
        $thn = $this->input->get('thn');
        $nip = $this->input->get('nip');

        $nilai = $this->m_api->get_skp_periodenip($thn, $nip);
        $data['nilai'] = $nilai;
        $response = array();
        $posts = array();
        //lopping data dari database
        if ($nilai != "Data tidak ditemukan") {
            ...
            {
                $posts[] = array(
                    "awal_periode_skp"  => $hasil->awal_periode_skp,
                    "akhir_periode_skp" => $hasil->akhir_periode_skp,
                    "nilai_skp"         => $hasil->nilai,
                    "jabatan"           => $hasil->jabatan,
                    "unit_kerja"        => $hasil->unitkerja,
                    "nip_atasan"        => $hasil->nip_atasan,
                    "nama_atasan"       => $hasil->nama_atasan
