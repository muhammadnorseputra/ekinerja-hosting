...
WHERE ( a.id_dd_user={$periode['id_dd_user']} OR a.id_dd_user_bawahan={$periode['id_dd_user']})
 UNION ALL
 SELECT d.tgl_entri, d.id_dd_user,d.id_dd_user_bawahan,d.id_opmt_turunan_skp,d.realisasi_waktu,d.realisasi_kualitas,d.realisasi_biaya,e.id_opmt_target_bulanan_skp,f.id_opmt_bulanan_skp,e.id_opmt_target_skp,f.tahun,d.kegiatan_turunan,' ',d.target_kuantitas,g.satuan_kuantitas,d.kualitas,d.target_waktu,d.biaya,'turunan' ket
...
FROM opmt_realisasi_harian_skp a
	WHERE month(a.tanggal)={$periode['bulan']} AND year(a.tanggal)={$periode['tahun']} AND a.id_dd_user={$id_dd_user} 	 GROUP BY a.id_opmt_target_bulanan_skp
...
count(case when a.verifikasi=100 AND a.id_opmt_target_bulanan_skp={$id} then a.id_opmt_realisasi_harian_skp else null end)kegiatan_sb,
count(case when a.verifikasi=90 AND a.id_opmt_target_bulanan_skp={$id} then a.id_opmt_realisasi_harian_skp else null end)kegiatan_baik,
count(case when a.verifikasi=75 AND a.id_opmt_target_bulanan_skp={$id} then a.id_opmt_realisasi_harian_skp else null end)kegiatan_cukup,
count(case when a.verifikasi=60 AND a.id_opmt_target_bulanan_skp={$id} then a.id_opmt_realisasi_harian_skp else null end)kegiatan_kurang,
count(case when a.verifikasi=50 AND a.id_opmt_target_bulanan_skp={$id} then a.id_opmt_realisasi_harian_skp else null end)kegiatan_buruk,
count(case when coalesce(a.verifikasi,0)=0 AND a.id_opmt_target_bulanan_skp={$id} then a.id_opmt_realisasi_harian_skp else null end)kegiatan_bv

from (select a.id_opmt_realisasi_harian_skp,b.id_opmt_target_bulanan_skp,a.verifikasi FROM opmt_realisasi_harian_skp a
INNER JOIN opmt_target_bulanan_skp b on a.id_opmt_target_bulanan_skp=b.id_opmt_target_bulanan_skp AND (b.id_dd_user={$id_user_bawahan} OR b.id_dd_user_bawahan={$id_user_bawahan}) AND a.turunan=0
 ...
 UNION ALL
 select a.id_opmt_realisasi_harian_skp,b.id_opmt_turunan_skp,a.verifikasi FROM opmt_realisasi_harian_skp a
...
INNER JOIN opmt_bulanan_skp c on b.id_opmt_bulanan_skp=c.id_opmt_bulanan_skp
WHERE a.id_opmt_target_skp={$id}
UNION ALL
...
INNER JOIN opmt_bulanan_skp d on b.id_opmt_bulanan_skp=d.id_opmt_bulanan_skp
WHERE a.id_opmt_target_skp={$id}
