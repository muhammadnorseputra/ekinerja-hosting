...
<i class="fa fa-dashboard fa-2x text-primary"/></a>';
            ...
            if ($jmlbulanan == 0) { 
                ...
                $row[] = '<i class="fa fa-ban fa-2x text-primary"/>';
            }
...
<i class="fa fa-pencil text-success"/></a>';
            ...
            if ($dt->tgl_entri == NULL) {
                ...
                $tglentri = date('D, d M Y h:i:s',strToTime($dt->tgl_entri));
            } 
            $row[] = $tglentri;
    ...
    public function update_bulanan_skp() {
        ...
        if ($update) {  
            $a['status'] = 1;
            $a['ket'] = 'Data Realisasi Bulanan berhasil di-Approve';
            $this->j($a);

            // Untuk Simpan Nilai SKP pada SILKa
            //$konek = @fSockOpen("127.0.0.1", 80);
            //var_dump($konek);
            //if ($konek) {              
		    ...
		    $url = "http://silka.bkppd.local/api/save_skpbln?nip=".$v['nip']."&th=".$v['tahun']."&bl=".$v['bulan']."&nil=".$v['nilai_skp']."&jab=".$jabatan."&uk=".$unitkerja."&lo=".$login_terakhir."&at=".$atasan;
