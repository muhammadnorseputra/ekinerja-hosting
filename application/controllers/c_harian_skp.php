...
//        $this->load->model('M_harian_skp_bawahan', 'skp_bawahan');
    ...
    public function ajax_bawahan() {
        ...
        $this->db->join("(
                SELECT tb.id_opmt_target_bulanan_skp as id,b.kegiatan_tahunan as kegiatan_bulanan,0 as ket FROM opmt_target_bulanan_skp tb
                INNER JOIN opmt_target_skp b ON tb.id_opmt_target_skp=b.id_opmt_target_skp
                INNER JOIN dd_user c ON tb.`id_dd_user` = `c`.`id_dd_user` 
                AND ( c.atasan_langsung = $id_user OR c.atasan_2 = $id_user OR c.atasan_3 = $id_user )
                UNION ALL
                SELECT id_opmt_turunan_skp as id,kegiatan_turunan,1 as ket FROM opmt_turunan_skp d
                INNER JOIN opmt_target_bulanan_skp e ON d.id_opmt_target_bulanan_skp=e.id_opmt_target_bulanan_skp
                INNER JOIN opmt_target_skp f on e.id_opmt_target_skp=f.id_opmt_target_skp
                INNER JOIN opmt_tahunan_skp g on g.id_opmt_tahunan_skp=f.id_opmt_tahunan_skp
                INNER JOIN dd_user h ON d.`id_dd_user` = `h`.`id_dd_user` 
                AND ( h.atasan_langsung = $id_user OR h.atasan_2 = $id_user OR h.atasan_3 = $id_user )
...
//        $id_user = $this->session->userdata('id_user');
        $dt_user = $this->db->query("SELECT id_dd_user FROM opmt_realisasi_harian_skp WHERE id_opmt_realisasi_harian_skp={$id}")->row_array();
        $id_user = $dt_user['id_dd_user'];	
...
SELECT a.id_opmt_target_bulanan_skp id,a.id_opmt_target_bulanan_skp,a.id_opmt_target_skp,d.kegiatan_tahunan kegiatan,a.turunan,a.target_kuantitas,c.satuan_kuantitas,a.realisasi_kualitas,a.target_waktu,a.biaya,'utama' ket
...
WHERE a.turunan=0
 UNION ALL
 SELECT d.id_opmt_turunan_skp,e.id_opmt_target_bulanan_skp,e.id_opmt_target_skp,d.kegiatan_turunan,' ',d.target_kuantitas,g.satuan_kuantitas,d.kualitas,d.target_waktu,d.biaya,'turunan' ket
