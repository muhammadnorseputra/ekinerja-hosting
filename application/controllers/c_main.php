...
public function sync_user(){
	$q="update  dd_user a ,tblrekaptk b
	SET a.nama=b.NAMA,a.jabatan=b.kodejab,a.unit_kerja=b.kodeunit,a.gol_ruang=b.kodegol WHERE  a.nip=b.nip";
	$sync=$this->db->query($q);
	if($sync){
	$hasil=$this->db->affected_rows();
if($hasil==0){
	$ket="Tidak ada user yang diupdate";
}else{
	$ket=$hasil." User terupdate";
}
		$a['status'] = 0;
      		$a['ket'] =$ket;
	}else{
		$a['status'] = 0;
      		$a['ket'] = $this->db->error_message();
	}
	$this->j($a);
}
    public function validasi_lama() {
        $this->load->model("M_login");
        $user = $this->input->post('username');
        $pass = $this->input->post('password');
        // $this->refresh_user($user);
    ...
    public function validasi() {
        // integrasi dengan SILKa Online
        // sebelum login, cek koneksi dengan silka online
        // jika terkoneksi, request json data pns untuk login, dan lakukan update data dd_user sesuai json silka
        // jika tidak terkoneksi, user dapat login, tapi dd_user tidak diupdate
        ...
        //$konek = false;
        $konek = @fSockOpen("36.91.72.99", 80); // pakai yang ini
        //$konek = @fSockOpen("localhost", 80);
        
        if ($konek AND ($user != "admin2" AND $user != "balangan1")) {
          //$this->session->set_flashdata('pesan', "Terhubung dengan SILKa Online");
          // cek nip pada silka
          $url = 'http://silka.bkppd-balangankab.info/api/cek_pns_ekinerja?nip='.$user;
          ...
          //var_dump($jml);
          /*
            "nip" => a, "nama" => b, "gol_ruang" => c, "jnsjab" => d, "jabatan" => e, 
            "unit_kerja" => f, "id_unker_kinerja" => g
          */

          // TO DO : proses data dari DES Web Service
          //if ($api->hasil !=  "Data Tidak Ditemukan") {
          if ($api) {
            foreach($api->hasil as $d) :
                //var_dump(decrypt($d->g));
                // Cek apakah NIP ditemukan pada SILKA
                if (decrypt($d->a) == $user) {
                    // Start validasi user ekinerja
                    $this->refresh_user($user);
                    ...
                    if ($validasi == 0) {
                        ...
                        $autosilka = $this->M_database->get_autosilka($user);
                        if ($autosilka == "Y") { 
                            // Update data tabel dd_user
                            $kdjabatan = $this->M_database->get_kodejabatan(decrypt($d->d), decrypt($d->e));
                            $kdunit = $this->M_database->get_kodeunit(decrypt($d->f));
                            //var_dump(decrypt($d->d));
                            //var_dump(decrypt($d->e));
                            $upuser = array(
                              'nama'         => decrypt($d->b),
                              'jabatan'      => $kdjabatan,
                              'unit_kerja'   => $kdunit,
                              'gol_ruang'    => decrypt($d->c)
                              );
                            //var_dump($upuser);
                            $up = $this->M_database->ubah_data('dd_user', 'nip', decrypt($d->a), $upuser);
                        }
                        ...
                        $this->session->set_userdata('unit_kerja', $usr['unitkerja']);
                        $this->session->set_userdata('nama_uker', $usr['unitkerja']);
                        $this->session->set_userdata('paswd', $pass);
                        ...
                        //var_dump($this->session->userdata('kodejab'));
                        //var_dump($this->session->userdata('jabatan'));
                        //var_dump($this->session->userdata('unit_kerja'));
                        //var_dump($this->session->userdata('paswd'));
                        
                        redirect('./');
                    }
                    // End validasi user ekinerja
                } 
                //else {
                    // NIP tidak ditemukan pada SILKa
                //    $this->session->set_flashdata('pesan', "NIP tidak ditemukan pada SILKa Online");
                //    redirect('./');
                //}                
                endforeach;
            } else {
        ...
        // JIka tidak terhubung dengan SILKa Online, user bisa login tapi tidak dilakukan update data profil dd_user
        } else {
                $validasi = $this->M_login->validasi($user, $pass);
                if ($validasi == 0) {
                    $this->session->set_flashdata('pesan', 'Username / Password Anda Salah !!!');
