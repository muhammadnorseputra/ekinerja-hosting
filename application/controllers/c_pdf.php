...
class C_pdf extends CI_Controller {
    ...
    public function cetak_target_tahunan_skp($id, $lokasi, $tanggal) {
        ...
        if ($cek['atasan_langsung'] == "") {
            ...
            $content = file_get_contents(base_url() . 'c_pdf/pdf_cetak_target_skp_tahunan/' . $id . '/' . $lokasi. '/' .$tanggal);
    ...
    public function pdf_cetak_target_skp_tahunan($id, $lokasi, $tanggal) {
        $id_user = $this->session->userdata('id_user');
...
WHERE id_dd_user='" . $atasan_langsung . "'")->row_array();
        ...
        $x['judul1'] = "";
        $x['judul2'] = "SASARAN KERJA PEGAWAI";
...
WHERE a.tahun={$thn} AND a.bulan={$bln} {$par_sql}";
    ...
    public function cetak_realisasi_tahunan_skp($id, $lokasi, $tanggal) {
        $id_user = $this->session->userdata('id_user');
        $params = array('type' => 'L', 'width' => '390', 'height' => '225');
        $this->load->library('html2pdf_lib', $params);
        
        //$content = file_get_contents(base_url() . 'c_pdf/pdf_realisasi_tahunan_skp/' . $id . '/' . $id_user . '/' . $lokasi);
        $content = file_get_contents(base_url() . 'c_pdf/pdf_realisasi_tahunan_skp/'.$id.'/'.$id_user.'/'.$lokasi.'/'.$tanggal);
        
        $filename = 'Realisasi Tahunan SKP.pdf';
        $this->html2pdf = new HTML2PDF('L', 'Legal', 'en', true, 'UTF-8', array(0, 0, 0, 0));
        $save_to = $this->config->item('upload_root');
        if ($this->html2pdf_lib->converHtml2pdf($content, $filename, $save_to)) {
            echo $save_to . '/' . $filename;
        } else {
    ...
    public function pdf_realisasi_tahunan_skp($id, $id_user, $lokasi, $tanggal) {
        $x['id'] = $id;
		...
		// harusnya null as biaya diganti dgn a.biaya as biaya, tapi error
		$x['real'] = $this->db->query("SELECT a.id_opmt_target_skp, a.id_opmt_tahunan_skp, a.kegiatan_tahunan, a.target_kuantitas, a.satuan_kuantitas, a.target_waktu, null as biaya, 
		a.id_opmt_target_skp_atasan, a.id_dd_user, a.id_dd_user_bawahan, a.angka_kredit_total, a.target_kuantitas_revisi, a.cek_80, a.target_waktu_revisi, a.biaya_revisi,
		b.satuan_kuantitas,sum(case when d.proses=0 then d.kuantitas else 0 end) realisasi_kuantitas,c.biaya biaya_realisasi,c.waktu waktu_realisasi,c.kualitas realisasi_kualitas
		FROM opmt_target_skp a 
			LEFT JOIN dd_kuantitas b ON a.satuan_kuantitas=b.id_dd_kuantitas 
			LEFT JOIN opmt_realisasi_skp c ON c.id_opmt_target_skp=a.id_opmt_target_skp
            LEFT JOIN opmt_realisasi_harian_skp d ON d.id_opmt_target_skp=a.id_opmt_target_skp AND d.proses=''
        ...
        $x['periode'] = $periode;
        $user = $this->db->query("SELECT * FROM dd_user WHERE id_dd_user={$id_user}")->row_array();
        $x['user'] = $user;
        $x['atasan'] = $this->db->query("SELECT * FROM dd_user WHERE id_dd_user={$user['atasan_langsung']}")->row_array();
    ...
    public function pdf_cetak_rekap($bulan, $tahun, $uker, $status) {
        ...
        if ($status == 1) {
            ...
            $par_status = " AND( nilai_skp =0 AND id_opmt_bulanan_skp>0)";
        } else {
...
INNER JOIN opmt_bulanan_skp c on b.id_opmt_bulanan_skp=c.id_opmt_bulanan_skp AND tahun={$tahun}
...
INNER JOIN opmt_bulanan_skp d on b.id_opmt_bulanan_skp=d.id_opmt_bulanan_skp AND tahun={$tahun}
WHERE a.id_dd_user={$id_user}
...
//            var_dump($listValues) . '<br>';
    ...
    public function cetak_realisasi_bulanan_skp($id, $lokasi) {
        $id_user = $this->session->userdata('id_user');
        $params = array('type' => 'L', 'width' => '350', 'height' => '300');
        $this->load->library('html2pdf_lib', $params);
        $content = file_get_contents(base_url() . 'c_pdf/pdf_realisasi_bulanan_skp/' . $id . '/' . $id_user . '/' . $lokasi);
        $filename = 'Realisasi Bulanan SKP.pdf';
        $this->html2pdf = new HTML2PDF('L', 'A4', 'en', true, 'UTF-8', array(0, 0, 0, 0));
        $save_to = $this->config->item('upload_root');
        if ($this->html2pdf_lib->converHtml2pdf($content, $filename, $save_to)) {
            echo $save_to . '/' . $filename;
    ...
    public function pdf_realisasi_bulanan_skp($id, $id_user, $lokasi) {
        $periode = $this->db->query("SELECT * FROM opmt_bulanan_skp WHERE id_opmt_bulanan_skp={$id}")->row_array();
        $x['periode'] = $periode;
        $x['disposisi'] = $this->db->query("SELECT * FROM opmt_disposisi WHERE month(tanggal_disposisi)={$periode['bulan']} AND id_dd_user={$id_user}")->result_array();
        $bulan = $this->db->query("SELECT * FROM opmt_bulanan_skp WHERE id_opmt_bulanan_skp={$id}")->row_array();
        $x['realisasi'] = $this->db->query("SELECT a.*,b.kuantitas realisasi_kuantitas
FROM(
...
ORDER BY a.id_opmt_target_bulanan_skp")->result_array();
        ...
        $x['user'] = $user;
        $x['atasan'] = $this->db->query("SELECT * FROM dd_user WHERE id_dd_user={$user['atasan_langsung']}")->row_array();
        $x['tugas_tambahan'] = $this->db->query("SELECT * FROM opmt_tugas_tambahan WHERE year(tanggal)={$periode['tahun']} AND month(tanggal)={$periode['bulan']} AND id_dd_user={$id_user}")->result_array();
        $x['kreatifitas'] = $this->db->query("SELECT * FROM opmt_kreatifitas_skp WHERE year(tanggal)={$periode['tahun']} AND month(tanggal)={$periode['bulan']} AND id_dd_user={$id_user}")->result_array();
//        $x['nilai_kreatifitas2'] = $this->db->query("SELECT * FROM opmt_kreatifitas_atasan WHERE tahun='" . date('Y', strtotime($periode['awal_periode_skp'])) . "' AND id_dd_user={$id_user}")->row_array();
        $x['lokasi'] = $this->db->query("select lokasi_spesimen from dd_spesimen WHERE id_dd_spesimen={$lokasi}")->row_array();
        $this->load->view('pdf/v_realisasi_bulanan_skp', $x);
    }

    public function cetak_harian($tahun,$bulan){
        $id_user = $this->session->userdata('id_user');
        $params = array('type' => 'L', 'width' => '350', 'height' => '300');
        $this->load->library('html2pdf_lib', $params);
        $link=base_url() . 'c_pdf/pdf_realisasi_harian_skp/'.$id_user.'/' . $tahun . '/' . $bulan;
      
        $content = file_get_contents($link);
        $filename = 'Realisasi Harian SKP.pdf';
        $this->html2pdf = new HTML2PDF('L', 'A4', 'en', true, 'UTF-8', array(0, 0, 0, 0));
        $save_to = $this->config->item('upload_root');
        if ($this->html2pdf_lib->converHtml2pdf($content, $filename, $save_to)) {
            echo $save_to . '/' . $filename;
    ...
    public function pdf_realisasi_harian_skp($id_user,$tahun,$bulan){
        ...
        $this->load->view('pdf/v_realisasi_harian_skp',$data);
    }
