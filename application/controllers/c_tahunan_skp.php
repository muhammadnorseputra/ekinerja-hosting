...
class C_tahunan_skp extends CI_Controller {
	...
	$dt_tahunan_skp=$this->db->query("SELECT akhir_periode_skp FROM opmt_tahunan_skp WHERE id_opmt_tahunan_skp={$id}")->row_array();
	$data['tahun']=date('Y',strtotime($dt_tahunan_skp['akhir_periode_skp']));
...
<i class="fa fa-pencil text-success fa-2x"/></a>';
            $link_hapus = '<a href="javascript:void(0)" onclick="hapus_tahunan_skp(' . $dt->id_opmt_tahunan_skp . ')">
<i class="fa fa-trash text-danger fa-2x"/></a>';
            $link_target_skp = '<a href="javascript:void(0)" onclick="target_tahunan_skp(' . $dt->id_opmt_tahunan_skp . ')">
<i class="fa fa-crosshairs text-warning fa-2x"/></a>';
            $link_realisasi = '<a href="javascript:void(0)" onclick="realisasi_tahunan_skp(' . $dt->id_opmt_tahunan_skp . ')">
<i class="fa fa-dashboard text-primary fa-2x"/></a>';
            $row = array();
            $row[] = $no;
            $row[] = "<h5 class='text-primary'>".date('d M Y', strtotime($dt->awal_periode_skp)) . " - " . date('d M Y', strtotime($dt->akhir_periode_skp))."</h5>";
            $row[] = $link_edit;
            
            // START untuk tombol HAPUS
            $jmltahunan = $this->M_database->getjmltahunan($dt->id_dd_user, $dt->id_opmt_tahunan_skp);
            //var_dump($jmltahunan);
            if ($jmltahunan == 0) { 
                $row[] = $link_hapus;
            } else {
...
<i class="fa fa-trash text-danger fa-2x"/></a>';
            ...
            $row[] = number_format($dt->angka_kredit_total,3);
            ...
            // START untuk tombol HAPUS
            $jml = $this->M_database->getjmlbulanan_pertahunan($dt->id_dd_user, $dt->id_opmt_target_skp);
            if ($jml == 0) { 
                $row[] = $link_hapus;
            } else {
                $row[] = "<i class='fa fa-ban fa-2x text-muted' />";
