...
class C_user extends CI_Controller {
    ...
    public function profile() {
        ...
        // Get Jabatan selain Admin : 2942
        $x['jabatan'] = $this->db->order_by('jabatan', 'ASC')->where_not_in('kodejab','2942')->from('tbljabatan a')->get()->result();
        $x['uker'] = $this->db->order_by('unitkerja', 'ASC')->from('tblstruktural a')->get()->result();
...
")->result_array();
    ...
    public function tambah_realisasi_tahunan_skp($id_tahun, $id) {
        $x['id'] = $id;
        $x['id_tahun'] = $id_tahun;
        $x['target']=$this->db->where('id_opmt_target_skp',$id)->get('opmt_target_skp')->row();
...
INNER JOIN opmt_tahunan_skp b oN a.id_opmt_tahunan_skp=b.id_opmt_tahunan_skp AND b.id_dd_user={$id_user}")->result_array();
        $x['parameter'] = $this->db->get('dc_parameter_bulan')->row_array();
        $harian_skp = $this->db->query("SELECT * FROM opmt_realisasi_harian_skp WHERE id_opmt_realisasi_harian_skp=" . $id)->row_array();
        $x['harian_skp'] = $harian_skp;
        $x['skp_bulanan'] = $this->db->query("SELECT * FROM(
SELECT a.id_opmt_target_bulanan_skp id,a.id_opmt_target_bulanan_skp,b.id_opmt_bulanan_skp,a.id_opmt_target_skp,b.tahun,d.kegiatan_tahunan kegiatan,a.turunan,a.target_kuantitas,c.satuan_kuantitas,a.realisasi_kualitas,a.target_waktu,a.biaya,'utama' ket
FROM (`opmt_target_bulanan_skp` a) 
INNER JOIN `opmt_bulanan_skp` b ON `b`.`id_opmt_bulanan_skp`=`a`.`id_opmt_bulanan_skp` AND b.id_dd_user={$id_user} AND tahun=" . date('Y', strtotime($harian_skp['tanggal'])) . "  AND bulan=" . date('n', strtotime($harian_skp['tanggal'])) . " 
INNER JOIN `opmt_target_skp` d ON `d`.`id_opmt_target_skp`=`a`.`id_opmt_target_skp` 
LEFT JOIN `dd_kuantitas` c ON `c`.`id_dd_kuantitas`=`a`.`satuan_kuantitas` 
WHERE a.turunan=0  
 UNION ALL
 ...
 FROM opmt_turunan_skp d
 INNER JOIN opmt_target_bulanan_skp e on e.id_opmt_target_bulanan_skp =d.id_opmt_target_bulanan_skp
 INNER JOIN opmt_bulanan_skp f on f.id_opmt_bulanan_skp=e.id_opmt_bulanan_skp AND f.id_dd_user={$id_user} AND tahun=" . date('Y', strtotime($harian_skp['tanggal'])) . "  AND bulan=" . date('n', strtotime($harian_skp['tanggal'])) . "    
 INNER JOIN `dd_kuantitas` g ON `g`.`id_dd_kuantitas`=`d`.`satuan_kuantitas` 
 ) a order by a.id_opmt_target_bulanan_skp")->result_array();
        $x['realisasi_skp'] = $this->db->query("SELECT * FROM opmt_realisasi_skp WHERE id_opmt_target_skp=" . $id_user)->row_array();
...
//        $data['turunan_skp'] = $this->db->query("SELECT * FROM opmt_turunan_skp WHERE id_opmt_turunan_skp='" . $id_turunan . "'")->row_array();
    ...
    public function target_bulanan_skp($id) {
        ...
        $data['parameter'] = $this->db->query("SELECT parameter_drop as 'drop' FROM dc_parameter_bulan")->row();
...
SELECT a.tgl_entri, a.id_dd_user,a.id_dd_user_bawahan,a.id_opmt_target_bulanan_skp id,a.id_opmt_target_bulanan_skp,b.id_opmt_bulanan_skp,a.id_opmt_target_skp,b.tahun,d.kegiatan_tahunan kegiatan,a.turunan,a.target_kuantitas,c.satuan_kuantitas,100 AS kualitas,a.target_waktu,a.biaya,'utama' ket
FROM (`opmt_target_bulanan_skp` a) 
...
WHERE ( a.id_dd_user={$id_user} OR a.id_dd_user_bawahan={$id_user})
 UNION ALL
 SELECT d.tgl_entri, d.id_dd_user,d.id_dd_user_bawahan,d.id_opmt_turunan_skp,e.id_opmt_target_bulanan_skp,f.id_opmt_bulanan_skp,e.id_opmt_target_skp,f.tahun,d.kegiatan_turunan,' ',d.target_kuantitas,g.satuan_kuantitas,d.kualitas,d.target_waktu,d.biaya,'turunan' ket
FROM opmt_turunan_skp d
...
FROM(
SELECT a.tgl_entri, a.id_dd_user,a.id_dd_user_bawahan,a.id_opmt_target_bulanan_skp id,a.realisasi_waktu,a.realisasi_kualitas,a.realisasi_biaya,a.id_opmt_target_bulanan_skp,b.id_opmt_bulanan_skp,a.id_opmt_target_skp,b.tahun,d.kegiatan_tahunan kegiatan,a.turunan,a.target_kuantitas,c.satuan_kuantitas,100 AS kualitas,a.target_waktu,a.biaya,'utama' ket,'0' ket2
FROM (`opmt_target_bulanan_skp` a) 
INNER JOIN `opmt_bulanan_skp` b ON `b`.`id_opmt_bulanan_skp`=`a`.`id_opmt_bulanan_skp` AND b.bulan={$bulan['bulan']} AND b.tahun={$bulan['tahun']}
LEFT JOIN `opmt_target_skp` d ON `d`.`id_opmt_target_skp`=`a`.`id_opmt_target_skp` 
LEFT JOIN `dd_kuantitas` c ON `c`.`id_dd_kuantitas`=`a`.`satuan_kuantitas` 
WHERE ( a.id_dd_user={$id_user} OR a.id_dd_user_bawahan={$id_user})
 UNION ALL
 SELECT d.tgl_entri, d.id_dd_user,d.id_dd_user_bawahan,d.id_opmt_turunan_skp,d.realisasi_waktu,d.realisasi_kualitas,d.realisasi_biaya,e.id_opmt_target_bulanan_skp,f.id_opmt_bulanan_skp,e.id_opmt_target_skp,f.tahun,d.kegiatan_turunan,' ',d.target_kuantitas,g.satuan_kuantitas,d.kualitas,d.target_waktu,d.biaya,'turunan' ket,'1' ket2
FROM opmt_turunan_skp d
 LEFT JOIN opmt_target_bulanan_skp e on e.id_opmt_target_bulanan_skp =d.id_opmt_target_bulanan_skp
 INNER JOIN opmt_bulanan_skp f on f.id_opmt_bulanan_skp=e.id_opmt_bulanan_skp AND f.bulan={$bulan['bulan']} AND f.tahun={$bulan['tahun']}
 LEFT JOIN `dd_kuantitas` g ON `g`.`id_dd_kuantitas`=`d`.`satuan_kuantitas`
...
FROM opmt_realisasi_harian_skp
	WHERE year(tanggal)={$periode['tahun']} AND month(tanggal)={$periode['bulan']} and id_dd_user={$id_user} GROUP BY id_opmt_target_bulanan_skp,turunan
...
ORDER BY a.id_opmt_target_bulanan_skp")->result_array();
    ...
    public function realisasi_bulanan_skp_old($id) {
...
ORDER BY a.id_opmt_target_bulanan_skp")->result_array();
    ...
    public function realisasi_bulanan_skp_2() {
        $data['cetak']=0;
        $tahun=date('Y');
        $bulan=date('n');
    
        $id_user = $this->session->userdata('id_user');
        $periode = $this->db->query("SELECT * FROM opmt_bulanan_skp WHERE tahun={$tahun} and bulan={$bulan} AND id_dd_user={$id_user}")->row_array();
        $data['periode'] = $periode;
        $data['disposisi'] = $this->db->query("SELECT * FROM opmt_disposisi WHERE month(tanggal_disposisi)={$bulan} AND id_dd_user={$id_user}")->result_array();
        // $bulan = $this->db->query("SELECT * FROM opmt_bulanan_skp WHERE id_opmt_bulanan_skp=".$periode['id_opmt_bulanan_skp'])->row_array();
        $data['id'] = $periode['id_opmt_bulanan_skp'];
        $data['realisasi'] = $this->db->query("SELECT a.*,b.kuantitas realisasi_kuantitas
FROM(
SELECT a.tgl_entri, a.id_dd_user,a.id_dd_user_bawahan,a.id_opmt_target_bulanan_skp id,a.realisasi_waktu,a.realisasi_kualitas,a.realisasi_biaya,a.id_opmt_target_bulanan_skp,b.id_opmt_bulanan_skp,a.id_opmt_target_skp,b.tahun,d.kegiatan_tahunan kegiatan,a.turunan,a.target_kuantitas,c.satuan_kuantitas,100 AS kualitas,a.target_waktu,a.biaya,'utama' ket,'0' ket2
FROM (`opmt_target_bulanan_skp` a) 
INNER JOIN `opmt_bulanan_skp` b ON `b`.`id_opmt_bulanan_skp`=`a`.`id_opmt_bulanan_skp` AND b.bulan=$bulan AND b.tahun=$tahun
LEFT JOIN `opmt_target_skp` d ON `d`.`id_opmt_target_skp`=`a`.`id_opmt_target_skp` 
LEFT JOIN `dd_kuantitas` c ON `c`.`id_dd_kuantitas`=`a`.`satuan_kuantitas` 
WHERE ( a.id_dd_user={$id_user} OR a.id_dd_user_bawahan={$id_user})
 UNION ALL
...
FROM opmt_turunan_skp d
 LEFT JOIN opmt_target_bulanan_skp e on e.id_opmt_target_bulanan_skp =d.id_opmt_target_bulanan_skp
 INNER JOIN opmt_bulanan_skp f on f.id_opmt_bulanan_skp=e.id_opmt_bulanan_skp AND f.bulan=$bulan AND f.tahun=$tahun
 LEFT JOIN `dd_kuantitas` g ON `g`.`id_dd_kuantitas`=`d`.`satuan_kuantitas`
WHERE ( d.id_dd_user={$id_user} OR d.id_dd_user_bawahan={$id_user}) 
) a LEFT JOIN(
SELECT id_opmt_target_bulanan_skp,turunan,sum(kuantitas) kuantitas
FROM opmt_realisasi_harian_skp
	WHERE year(tanggal)=$tahun AND month(tanggal)=$bulan AND id_dd_user={$id_user} GROUP BY id_opmt_target_bulanan_skp,turunan
)b on a.id=b.id_opmt_target_bulanan_skp AND a.ket2=b.turunan
ORDER BY a.id_opmt_target_bulanan_skp")->result_array();
        $data['spesimen'] = $this->db->query('SELECT * FROM dd_spesimen')->result_array();
    ...
    public function realisasi_tahunan_skp($id) {
        ...
        $id_user = $this->session->userdata('id_user');
        $x['id_user'] = $id_user;
        $q_backup = "SELECT a.*,b.satuan_kuantitas, 
...
GROUP BY a.id_opmt_target_skp";
        $q_real = "SELECT a.*,b.satuan_kuantitas,sum(case when d.proses=0 then d.kuantitas else 0 end)realisasi_kuantitas,c.biaya biaya_realisasi,c.waktu waktu_realisasi,c.kualitas realisasi_kualitas
		FROM opmt_target_skp a 
			LEFT JOIN dd_kuantitas b ON a.satuan_kuantitas=b.id_dd_kuantitas 
			LEFT JOIN opmt_realisasi_skp c ON c.id_opmt_target_skp=a.id_opmt_target_skp
                        LEFT JOIN opmt_realisasi_harian_skp d ON d.id_opmt_target_skp=a.id_opmt_target_skp AND d.proses=''
			WHERE a.id_opmt_tahunan_skp='" . $id . "' GROUP BY a.id_opmt_target_skp";
        $real = $this->db->query($q_real)->result_array();
        $x['real'] = $real;
...
//        $x['tugas_tambahan'] = $this->db->query("SELECT * FROM opmt_tugas_tambahan WHERE year(tanggal)='" . date('Y', strtotime($periode['awal_periode_skp'])) . "' AND id_dd_user={$id_user}")->result_array();
        $x['tugas_tambahan'] = $this->db->query("SELECT * FROM opmt_tugas_tambahan2 WHERE tahun='" . date('Y', strtotime($periode['awal_periode_skp'])) . "' AND id_dd_user={$id_user}")->result_array();
    ...
    public function simpan_realisasi_tahunan() {
        $post = $this->input->post();
        $cek = $this->db->from('opmt_realisasi_tahunan_skp')->where('id_opmt_tahunan_skp', $post['id_opmt_tahunan_skp'])->get()->row();
        if (count($cek) > 0) {
            $this->db->where('id_opmt_realisasi_tahunan_skp', $cek->id_opmt_realisasi_tahunan_skp);
            unset($post->id_opmt_realisasi_tahunan_skp);
            $proses = $this->db->update('opmt_realisasi_tahunan_skp', $post);
        } else {
            $proses = $this->db->insert('opmt_realisasi_tahunan_skp', $post);
            return $proses;
        }
    }
...
INNER JOIN opmt_target_skp b on a.id_opmt_tahunan_skp=b.id_opmt_tahunan_skp AND a.id_dd_user=(select atasan_langsung from dd_user where id_dd_user={$id_user}) AND year(a.akhir_periode_skp)=$tahun")->result_array();
        $data['periode'] = $periode;
...
where kodeunit='{$data_user['kodeunit']}' AND a.tahun={$tahun}")->result_array();
    ...
    public function tambah_target_bulanan_skp($id) {
        ...
        $data['dt_skp_tahunan'] = $this->db->query("SELECT a.*, b.* FROM opmt_tahunan_skp a 
...
WHERE a.id_dd_user={$id_user}")->result_array();
    ...
    public function aksi_tambah_periode_bulanini() {
        $id_user = $this->session->userdata('id_user');
        $nip = $this->session->userdata('nip');
        $bulanini = date('m');
        $tahunini = date('Y');

        $cek = $this->db->from("opmt_bulanan_skp")->where("bulan", $bulanini)->where("tahun", $tahunini)->where("id_dd_user", $this->session->userdata('id_user'))->count_all_results();
        if ($cek > 0) {
            echo "<div class='alert alert-info' role='alert'>Periode SKP Bulan ini sudah ada</div>"; 
            die();
        } else {
            $tgl = $this->M_database->datetime_saatini();
            $data = array(
              'id_dd_user'   => $id_user,
              'nip'          => $nip,
              'bulan'        => $bulanini,
            ...
            $cek = $this->M_database->tambah_data('opmt_bulanan_skp', $data);
    ...
    public function aksi_harian_skp() {
        ...
        $tgl = $this->M_database->datetime_saatini();
        $p->tgl_entri_laporan = $tgl;
        
        // JIKA Menggunakan batas waktu 3 Hari (H+2)
        /*
        // Set data tanggal, H, H-1 atau H-2
        $hariini = date('Y-m-d');
        if ($p->tanggal == '1') {
            $p->tanggal = $hariini;
        } else if ($p->tanggal == '2') {
            $p->tanggal = date('Y-m-d', strtotime('-1 days', strtotime($hariini)));
        } else if ($p->tanggal == '3') {
            $p->tanggal = date('Y-m-d', strtotime('-2 days', strtotime($hariini)));
        }
        */
        
        // cek jumlah laporan unt tanggal tersebut
        $cekjmllap = $this->M_database->getjmllaporanpertanggal($p->id_dd_user, $p->tanggal);
        $tgllap = $this->M_database->get_tgllapharian($p->id_dd_user, $p->id_opmt_realisasi_harian_skp);

        // get tanggal target bulanan yang terkait dgn harian ini
        $tgltargetbulanan = $this->M_database->get_tglentribulanan($p->id_opmt_target_bulanan_skp);
        $tglentrilap = $this->M_database->get_tglentrilapharian($p->id_dd_user, $p->id_opmt_realisasi_harian_skp);

        if (($cekjmllap < 7) OR ($p->tanggal == $tgllap)) { // batasi jumlah laporan harian maksimal 3 laporan per hari/tanggal
            if ($p->id_opmt_realisasi_harian_skp == '') {
                ...
                // update jumlah realisasi waktu bulanan, sesuaikan dengan jumlah realisasi harian
                $this->sesuaikanwaktu($p->id_dd_user, $p->id_opmt_target_bulanan_skp, $p->turunan);
                // end
            } else {
                // untuk Edit harian, tanggal entri bulanannya harus lebih kecil dibanding tanggal entri hariannya
                $p->tgl_entri_laporan = $tglentrilap;
                
                // Batasi edit harian hanya jika tgl entri hariannya setelah target bulanan dibuat (tgl create skp bulanannya) 
                //if ($tgltargetbulanan < $tglentrilap) {
            ...
            $a['status'] = 0;
            $a['ket'] = 'GAGAL ::: Jumlah laporan harian tanggal '.date('d F Y', strtotime($p->tanggal)).' sudah melewati batas maksimal';
            $this->j($a);
        }
            
    }
    
    public function sesuaikanwaktu($id_dd_user, $id_opmt_target_bulanan_skp, $turunan) {
        if ($turunan == 0) { // tidak ada turunan (kegiatan Non Tahapan)
            $jmlharian = $this->M_database->getjmlharian($id_dd_user, $id_opmt_target_bulanan_skp);
            
            $where = array(
              'id_opmt_target_bulanan_skp'  => $id_opmt_target_bulanan_skp,
              'id_dd_user'                  => $id_dd_user
              );

            $data = array(
              'realisasi_waktu'      => $jmlharian
            );
            $this->M_database->edit_opmt_target_bulanan_skp($where, $data);   
        } else if ($turunan == 1) {
            $jmlharian = $this->M_database->getjmlharian($id_dd_user, $id_opmt_target_bulanan_skp);
            
            $where = array(
                // pada tabel opmt_turunan_skp
                // isi id_opmt_turunan_skp = id_opmt_target_bulanan_skp yang ada pada tabel opmt_realisasi_harian_skp
                'id_opmt_turunan_skp'  => $id_opmt_target_bulanan_skp, 
                'id_dd_user'           => $id_dd_user
            );
            ...
            $this->M_database->edit_opmt_turunan_skp($where, $data);
        }
    }
    
    public function sesuaikanwaktutarget($id_dd_user, $id_opmt_target_bulanan_skp, $turunan) {
        if ($turunan == 0) { // tidak ada turunan (kegiatan Non Tahapan)
            $jmlharian = $this->M_database->getjmlharian($id_dd_user, $id_opmt_target_bulanan_skp);
    ...
    public function batal_revisi_target(){
         //1 kuantitas 2 waktu 3 
        $post=$this->input->post();
        $id_opmt_target_skp=$post['id_opmt_target_skp'];
        $id=$post['id'];
        switch($id){
            case 1:
            $kolom="target_kuantitas_revisi";
            break;
            case 2:
            $kolom="target_waktu_revisi";
            break;
            case 3:
            $kolom="biaya_revisi";
            break;
        }
        $dataUpdate=array($kolom=>0);
        $this->db->where('id_opmt_target_skp',$id_opmt_target_skp);
       $update= $this->db->update('opmt_target_skp',$dataUpdate);
       if($update){
           echo 'update';
       }
    }
    public function aksi_realisasi_skp() {
        ...
        if ($p->id_opmt_realisasi_skp == '0') {
            ...
            $cek = $this->M_database->tambah_data('opmt_realisasi_skp', array('id_opmt_target_skp'=>$p->id_opmt_target_skp, 'biaya'=>$p->biaya, 'waktu'=>$p->waktu));
        } else {
            $cek = $this->M_database->ubah_data('opmt_realisasi_skp', 'id_opmt_realisasi_skp', $p->id_opmt_realisasi_skp, array('waktu'=>$p->waktu));
        }
        
        isset($p->target_kuantitas_revisi)?$dataUpdate['target_kuantitas_revisi']=$p->target_kuantitas_revisi:'';
        isset($p->target_waktu_revisi)?$dataUpdate['target_waktu_revisi']=$p->target_waktu_revisi:'';
        isset($p->biaya_revisi)?$dataUpdate['biaya_revisi']=$p->biaya_revisi:'';
        
        if(isset($dataUpdate)){
            $this->db->where('id_opmt_target_skp',$p->id_opmt_target_skp);
            $this->db->update('opmt_target_skp',$dataUpdate);
        }
        if ($cek) {
            $a['status'] = 1;
    ...
    public function aksi_turunan_skp() {
        ...
        if ($p->id_opmt_turunan_skp == '') {
            ...
            $p->id_dd_user = $this->session->userdata('id_user');
            $p->realisasi_waktu=$p->target_waktu;
    ...
    public function aksi_biaya_skp() {
        ...
        if (isset($p->id_opmt_turunan_skp)) {
			$this->db->select('a.*,b.id_opmt_target_skp');
			$dt=$this->db->where('id_opmt_turunan_skp',$p->id_opmt_turunan_skp)->join('opmt_target_bulanan_skp b','a.id_opmt_target_bulanan_skp=b.id_opmt_target_bulanan_skp','INNER')->get('opmt_turunan_skp a')->row();
        //update biaya target tahunan skp 230219
        $id_opmt_target_skp=$dt->id_opmt_target_skp;
		if(!isset($p->realisasi_waktu)){
       $p->realisasi_waktu=$dt->target_waktu;
		}
            if ($p->id_opmt_turunan_skp == "") {
                unset($p->id_opmt_turunan_skp);
                $cek = $this->M_database->tambah_data('opmt_turunan_skp', $p);
		...
		$dt=$this->db->where('id_opmt_target_bulanan_skp',$p->id_opmt_target_bulanan_skp)->get('opmt_target_bulanan_skp')->row();
        //update biaya target tahunan skp 230219
        $id_opmt_target_skp=$dt->id_opmt_target_skp;
	if(!isset($p->realisasi_waktu)){    
	$p->realisasi_waktu=$dt->target_waktu;
        ...
        $q="select id_opmt_target_skp,sum(biaya)biaya from(
            select id_opmt_target_skp,sum(realisasi_biaya)biaya from opmt_target_bulanan_skp a
            where  id_opmt_target_skp=$id_opmt_target_skp
            group by id_opmt_target_skp
            UNION ALL
            select id_opmt_target_skp,sum(b.realisasi_biaya) from opmt_target_bulanan_skp a
            inner join opmt_turunan_skp b on b.id_opmt_target_bulanan_skp=a.id_opmt_target_bulanan_skp
            where  id_opmt_target_skp=$id_opmt_target_skp
            group by id_opmt_target_skp
            )a
            group by id_opmt_target_skp";
            $biayaTahunan=$this->db->query($q)->row();
            
            $cek2=$this->db->where('id_opmt_target_skp',$id_opmt_target_skp)->get('opmt_realisasi_skp')->row();
            if(count($cek2)>0){
                $this->db->where('id_opmt_realisasi_skp',$cek2->id_opmt_realisasi_skp)->update('opmt_realisasi_skp',array('biaya'=>$biayaTahunan->biaya));
            }else{
                $this->db->insert('opmt_realisasi_skp',array('biaya'=>$biayaTahunan->biaya));
            }
    ...
    public function aksi_target_bulanan_skp() {
        ...
        if ($p->id_opmt_target_bulanan_skp == '') {
            ...
            // UNTUK ENTRI AWAL, realisasi waktu dijadikan 0
            $p->realisasi_waktu=0;
            ...
            // UNTUK EDIT TARGET, realisasi waktu tidak dirobah, tetap data sebelumny
            //$p->realisasi_waktu=$p->target_waktu;
...
<i class="fa fa-dashboard text-primary"/></a>';
    ...
    public function ubah_target_tahunan_skp($id) {
       $id_user = $this->session->userdata('id_user');
        ...
        $x['dt_kuantitas'] = $this->db->order_by('satuan_kuantitas', 'ASC')->get('dd_kuantitas')->result_array();
		$target_tahunan_skp=$this->db->where('id_opmt_target_skp', $id)->join('opmt_tahunan_skp b', 'a.id_opmt_tahunan_skp=b.id_opmt_tahunan_skp', 'LEFT')->get('opmt_target_skp a')->row_array();
        $x['target_tahunan_skp'] = $target_tahunan_skp;
		 $tahun = date('Y', strtotime($target_tahunan_skp['akhir_periode_skp']));
...
where kodeunit='{$data_user['kodeunit']}'")->result_array();
    ...
    public function hapus_harian() {
        /*
        // SOURCE CODE ORI
        // hapus harian tidak menghitung/menyesuaikan jumlah realisasi harian setelah dihapus
        $id = $this->input->post('id');
        ...
        */
        ...
        $id = $this->input->post('id');
        // update jumlah realisasi waktu bulanan, sesuaikan dengan jumlah realisasi harian
        $id_dd_user = $this->M_database->get_dduser($id);
        $id_opmt_target_bulanan_skp = $this->M_database->get_opmt_target_bulanan_skp($id);        
        $turunan = $this->M_database->get_turunan($id);
        // end
        ...
        // sesuaikan waktu realisasi bulanan
        $this->sesuaikanwaktu($id_dd_user, $id_opmt_target_bulanan_skp, $turunan);
        $this->sesuaikanwaktutarget($id_dd_user, $id_opmt_target_bulanan_skp, $turunan);
