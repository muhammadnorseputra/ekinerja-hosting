...
class M_detail_kegiatan_jabatan2 extends CI_Model {
    ...
    private function _get_datatables_query($kodejab,$jabatan) {
if(is_numeric($kodejab)){
$parx=" AND c.kodejab={$kodejab}";
}else{
$parx=" AND c.kodejab=9999";
        ...
        $this->db->join('opmt_kegiatan_jabatan c', "a.id_opmt_kegiatan_jabatan=c.id_opmt_kegiatan_jabatan {$parx}", 'INNER');
        $i = 0;
    ...
    function get_datatables($kodejab,$jabatan) {
        $this->_get_datatables_query($kodejab,$jabatan);
        if ($_POST['length'] != -1)
    ...
    function count_filtered($kodejab,$jabatan) {
        $this->_get_datatables_query($kodejab,$jabatan);
        $query = $this->db->get();
        return $query->num_rows();
    }

    public function count_all($kodejab,$jabatan) {
