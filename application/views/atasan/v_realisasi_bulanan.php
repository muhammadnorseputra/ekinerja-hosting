<style>
    ...
    }.judul{font-weight: normal;}td{vertical-align: middle !important;}
</style>
<div style="padding:10px;">
    <div class="row">
        <div class="col-lg-12" style="text-align:center;font-weight:bold;font-size:14px;">
            <span style="text-transform: capitalize;">REALISASI SKP BULAN <?= bulan($periode['bulan']) ?>  TAHUN <?= $periode['tahun'] ?></span><br/>
            <span class='text text-danger'><?= strtoupper($periode['nama']) ?></span>
        </div>
    </div>
    ...
    <table class="table" id="tbl_realisasi">
        <thead class="ui-state-default">
            <tr>
                ...
                <td class="tengah" rowspan="2">Tgl.<br/>Membuat Target</td>
        ...
        <tbody>
            ...
            foreach ($realisasi as $real_arr) {
                // cek apakah terdapat realisasi yang lebih besar dari target, jika ya tambahkan variabel cek
		        //if ((($real_arr['target_kuantitas']==0)&&($real_arr['realisasi_kuantitas']>=$real_arr['target_kuantitas']))||(($real_arr['target_kuantitas']>0)&&($real_arr['realisasi_kuantitas']>$real_arr['target_kuantitas']))){$cek++;}
...
//echo $real_arr['id_dd_user_bawahan'].'tes'.$this->session->userdata('id_user') ;
                ...
                $nilai_kuantitas = $target_kuantitas == 0 ? 0 : ((is_null($realisasi_kuantitas)?0:$realisasi_kuantitas) / $target_kuantitas) * 100;
                ...
                <tr class="<?= $real_arr['ket'] == 'utama' ? 'judul' : '' ?><?=(($real_arr['realisasi_kuantitas']>$real_arr['target_kuantitas'])&&($real_arr['target_kuantitas']>0))?' danger':''?>">
                    ...
                    <td class="tengah" width='120'>                    
                        <?php
                        
                            if ($real_arr['tgl_entri'] == NULL) {
                                $tglentri = '';
                            } else {
                                $tglentri = tglwaktu_indo($real_arr['tgl_entri']);
                            } 
...
$i++;
            ...
            ?>
            <tr style="background-color:#dff0d8;text-align: left;font-weight: bold;">
                <td colspan="12">Nilai SKP :
                    <code>
                    <?= $i == 0 ? $total_nilai = 0 : number_format($total_nilai = array_sum($ttl_nilai) / $k, 2) ?>
                    </code>   
                </td>
            </tr>
            <tr>
                <td colspan="12" style="font-weight:bold;">Disposisi Tugas ( Status : Selesai Dilaksanakan dari Atasan )</td>
            </tr>
            <?php
            ...
            foreach ($disposisi as $arr2) {
                ?>
                <tr>
                    <td colspan="12" align="left" ><?= $arr2['kegiatan'] ?></td>
                    <?php if ($i == 0) { ?>
            ...
            ?>
            <tr style="background-color:#dff0d8;text-align: left;font-weight: bold;">
                <td colspan="12">TOTAL NILAI SKP :
                <h4><code>
                    <?php echo number_format($nilai_bulanan = $total_nilai + $nilai_dsp, 2);?>
                </code></h4>
    ...
    <?php
    // tampilkan tombol Approve jika semua realisasi kurang atau sama dengan target, atau total nilai SKP <= 100
    if(($cek == 0) AND ($nilai_bulanan <= 100)) {
    ?>
...
//                var $message = $('<div></div>').load('c_pdf/cetak_jfk/' + jenis );
                ...
                $.post('c_bulanan_skp/update_bulanan_skp', {id: id, nilai: nilai}, function (data) {
                    if (data.status == 1) {
                        ...
                        BootstrapDialog.show({
                            title: 'INFORMASI',
                            message: '<center>' + data.ket + '</center>'
                        });
                        //alert(data.ket);
    ...
    // fungsi modal untuk menampilkan warning kalau terdapat realisasi besar dari target, atau total nilai SKP lebih dari 100
    function alert() {
        var cek=<?=$cek?>;
        if(cek>0){
        BootstrapDialog.show({
            title: 'INFORMASI',
            message: '<b><center>Terdapat Kegiatan Bulanan yang Membutuhkan Penyesuaian Kuantitas<br/>Atau Total Nilai SKP lebih dari 100</b>,<br/>Sehingga kinerja bulanan bawahan ini tidak dapat di-APPROVE<br/> Silahkan Cek Kembali Kuantitas Target dan Realisasi Kegiatan Bawahan Anda</center>'
        });
    }
    }
    alert();
