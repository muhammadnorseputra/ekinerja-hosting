...
if (isset($harian_skp)) {
    ...
    $id = $harian_skp['id_opmt_target_bulanan_skp'];
    $ket = $harian_skp['turunan'] == 0 ? 'utama' : 'turunan';
        ...
        <td colspan="3">
            <select class="form-control" disabled name="id_opmt_target_bulanan_skp">
                ...
                foreach ($skp_bulanan as $arr) {
                    ?>    
                    <option value="<?= $arr['id'] . '-' . $arr['ket'] ?>" <?= $arr['id'] . '-' . $arr['ket'] == $id . '-' . $ket ? 'selected' : '' ?>><?= $arr['kegiatan'] ?></option>
                <?php }
                ?>
            </select>
        </td>
