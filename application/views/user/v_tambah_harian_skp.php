...
<form id="frm_harian_skp" method="post">
    <table class="table">
        <tr>
            <td style="width:160px !important;">Tanggal</td>
            <td> : </td> 
            <td colspan="3">
                <?php
                    // Jika menggunakan pembatasan H+2
                    $hariini = date('d-m-Y');
        ...
        <tr>
            <td>Klasifikasi SKP Bulanan Ini</td>
            <td> : </td> 
            <td colspan="3">
...
<script>
    ...
    } else if ($('[name="kuantitas"]').val() == 0) {
    alert('Kuantitas Tidak Boleh Kosong / null');
    }else {
    $.ajax({
    type: "POST",
    ...
    dateFormat: 'yy-mm-dd',
            autoclose: true,
    <?php 
        if ($parameter['parameter_bulan'] == 1) { // 1.Berarti pengaturan disabled tahun bulan diuncentang
    ?>
        //minDate: -2,
        //minDate: "-10d",
        hideIfNoPrevNext: true, //Biasanya tautan sebelumnya dan berikutnya dinonaktifkan jika tidak berlaku (lihat opsi minDate dan maxDate). Anda dapat menyembunyikannya sama sekali dengan menyetel atribut ini ke true
        stepMonths: 1, // Atur berapa bulan untuk berpindah saat mengklik tautan sebelumnya/berikutnya.
        maxDate: 0,
    <?php 
        } 
    ?>
