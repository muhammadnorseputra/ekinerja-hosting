...
if (isset($target_bulanan_skp)) {
    ...
    $realisasi_waktu = $target_bulanan_skp['realisasi_waktu'];
    ...
    <table class="table">
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" class="form-control isi" onkeyup="return selainNol(event)" onkeypress="return hanyaAngka(event)" name="target_kuantitas" value="<?= $target_kuantitas ?>"></td>
            ...
            <td>
                <select class="form-control" name="target_waktu">
                    <?php 
                        ...
                        if (($realisasi_waktu == 0) OR ($realisasi_waktu == "")) {
                            $realisasi_waktu = 31;
                        }
                        for ($i = 1; $i <= $realisasi_waktu; $i++) {
                        ?>
                            <option value="<?= $i ?>" <?= $realisasi_waktu == $i ? 'selected' : '' ?>><?= $i ?> </option>
                        <?php 
                        } 
                    ?>
                </select>
...
<script>
    ...
    function selainNol(evt) {
        ...
        if (frm_target_bulanan_skp.target_kuantitas.value == "0") {  
                alert("Target tidak boleh NOL");          
                frm_target_bulanan_skp.target_kuantitas.value = "1";
                frm_target_bulanan_skp.target_kuantitas.focus;
            return false;
        } 
        
        if (frm_target_bulanan_skp.target_kuantitas.value == "") {  
